...
def get_ip():
    ...
    data = []
    cursor = conn.cursor()
    sql = 'SELECT ip from dns_available WHERE visit_time is NULL or visit_time = "" '
    try:
        cursor.execute(sql)
    except:
    ...
    try:
        for result in results:
            if result['domain'] == 'www.baidu.com':
                sql = ' UPDATE dns_available SET baidu = "%s",visit_time = "%s" WHERE ip = "%s" ' % (result['domain_info'],current_time,result['ip'])
                cursor.execute(sql)
                
            elif result['domain'] == 'www.ifeng.com':
                sql = ' UPDATE dns_available SET ifeng = "%s",visit_time = "%s" WHERE ip = "%s" '  % (result['domain_info'],current_time,result['ip'])
                cursor.execute(sql)
                
            elif result['domain'] == 'www.163.com':
                sql = ' UPDATE dns_available SET wangyi = "%s",visit_time = "%s" WHERE ip = "%s" ' % (result['domain_info'],current_time,result['ip'])
                cursor.execute(sql)
                
            elif result['domain'] == 'www.hitwh.edu.cn':
                sql = ' UPDATE dns_available SET hitwh = "%s",visit_time = "%s" WHERE ip = "%s" ' % (result['domain_info'],current_time,result['ip'])
                cursor.execute(sql)
                
            else:
                sql = ' UPDATE dns_available SET sina = "%s",visit_time = "%s" WHERE ip = "%s" ' % (result['domain_info'],current_time,result['ip'])
                cursor.execute(sql)
        conn.commit()
    except:
        print 'Update failed'
...
def result_timeout(result_diffrences):
    ...
    try:
        for result in result_diffrences:
            ...
            sql = ' UPDATE dns_available SET visit_time = "%s" WHERE ip = "%s" ' % (current_time,result)
            cursor.execute(sql)
        conn.commit()
    except:
